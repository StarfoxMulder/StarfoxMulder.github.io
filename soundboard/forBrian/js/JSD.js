...
//// IF false && any Audio is currently playing in the DOM \\\\
  if(soundOverPlay == false && Audio(soundArray[cSID].soundLink).currentTime < 0 || Audio(bonusTracks[cSID].soundLink).currentTime < 0 ) {
	for (var x = 0; x < soundArray.length; x++) {
		Audio(soundArray[x].soundLink).pause();
		Audio(soundArray[x].soundLink).currentTime = 0;
	}
	for (var q = 0; q < bonusTracks.length; q++) {
		Audio(soundArray[q].soundLink).pause();
		Audio(soundArray[q].soundLink).currentTime = 0;
	}
		if ($(this).attr('id') == 'b0') {
